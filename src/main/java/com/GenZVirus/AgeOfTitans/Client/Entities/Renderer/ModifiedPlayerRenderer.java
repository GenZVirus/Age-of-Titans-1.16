...
import com.GenZVirus.AgeOfTitans.Client.Entities.Renderer.Layers.ModifiedHeldItemLayer;
...
@OnlyIn(Dist.CLIENT)
public class ModifiedPlayerRenderer extends LivingRenderer<AbstractClientPlayerEntity, PlayerModel<AbstractClientPlayerEntity>> {
	public ModifiedPlayerRenderer(EntityRendererManager renderManager) {
	...
	public ModifiedPlayerRenderer(EntityRendererManager renderManager, boolean useSmallArms) {
		...
		this.addLayer(new ModifiedHeldItemLayer<>(this));
	...
	private static BipedModel.ArmPose func_241741_a_(AbstractClientPlayerEntity p_241741_0_, Hand p_241741_1_) {
		...
		if (itemstack.isEmpty() || itemstack.getItem().equals(ItemInit.TITAN_HEART.get())) {
	...
	protected void renderName(AbstractClientPlayerEntity entityIn, ITextComponent displayNameIn, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn) {
		...
		if (d0 < 100.0D) {
			...
			if (scoreobjective != null) {
				...
				super.renderName(entityIn, (new StringTextComponent(Integer.toString(score.getScorePoints()))).appendString(" ").append(scoreobjective.getDisplayName()), matrixStackIn, bufferIn,
						packedLightIn);
	...
	public void renderRightArm(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int combinedLightIn, AbstractClientPlayerEntity playerIn) {
		if(!playerIn.getHeldItemMainhand().getItem().equals(ItemInit.TITAN_HEART.get()))
