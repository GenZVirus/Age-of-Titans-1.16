...
public class WitheringDaggerSetBonusEventsHandler {
	...
	public static void SetBonus(LivingDamageEvent event) {
		...
		if (attacker.getHeldItemMainhand().getItem().equals(ItemInit.WITHERING_TITAN_STEEL_DAGGER.get())) {
			if (attacker.getHeldItemOffhand().getItem().equals(ItemInit.WITHERING_TITAN_STEEL_DAGGER.get())) {
		...
		} else if (attacker.getHeldItemOffhand().getItem().equals(ItemInit.WITHERING_TITAN_STEEL_DAGGER.get())) {
			if (event.getEntityLiving().isPotionActive(Effects.WITHER)) {
				int amplifier = event.getEntityLiving().getActivePotionEffect(Effects.WITHER).getAmplifier();
				if (amplifier < 4) {
					amplifier++;
